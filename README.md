...
- **Версионирование ресурсов** — реестр инструментов/воркфлоу/приложений с откатами (см. ниже)
...
## 🗃️ Реестр ресурсов (версии и откаты)

- PWA: `/pwa/registry.html` — просмотр текущих версий и откаты
- REST: `GET /api/v1/registry/tools|workflows|apps`, `GET /api/v1/registry/{cat}/{key}/versions`, `POST /api/v1/registry/{cat}/{key}/rollback`
- Хранилище: файл `data/registry.json` по умолчанию или Redis при `REGISTRY_BACKEND=redis`
...
- [Реестр ресурсов и откаты](docs/registry.md)
